...
export function filterDiets(diet) {
    ...
    if (diet === "all") { // Si el "select" de la pagina esta en "Select diet" va a entrar en esta condicion
      return ({
      ...
      if(filter_recipes.length <= 0){
        return ({
          type: FILTER_DIETS, // Le digo el tipo de acción con el que será llamada
          payload: [{title: "No diet", summary: `We do not have ${diet} recipes.`}], // El payload es el contenido que devuelve la acción 
        });
      }
...
export function filterOrder(order) { // Solo devuelvo el string que dice el tipo de orden
  return {
...
export function filterOrderScore(order) { // Solo devuelvo el string que dice el tipo de orden
  return {
...
export function detailRecipe(id) {
  return async function (dispatch) {
    ...
    if (id.toString().includes("-")) { // Si el "id" de la receta incluye "-" (UUID)
